...
class ParseResult:
    ...
    def failure(self, error):
        if not self.error or self.advance_count == 0:  # have not advanced since
...
class Parser:
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'VAR','IF', 'FOR', 'WHILE', 'FUN', int, float, identifier, '+', '-' or '(' or 'NOT'"
            ))
...
class Interpreter:
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        elif node.op_tok.matchs(TT_KEYWORD, 'NOT'):
